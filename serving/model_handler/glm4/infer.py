...
def _preprocess_messages_for_chatglm4(messages: List[Dict]):
    item = messages.pop(-1)
...
class ModelEngine(ModelEngine_Base):
    ...
    def chat_stream(self, messages: List[Dict], **kwargs):
        ...
        role, query, history = _preprocess_messages_for_chatglm4(messages)
        for response, history in self.get_model().stream_chat(self.tokenizer, query=query, role=role, history=history,
                                                              with_postprocess=False, **default_kwargs):
            chunk.step(response)
            if chunk.can_output():
    ...
    def chat(self, messages: List[Dict], **kwargs):
        ...
        args_process.build_args(default_kwargs)
        role, query, history = _preprocess_messages_for_chatglm4(messages)
        response, history = self.model.chat(self.tokenizer, query=query, role=role, history=messages,
                                            with_postprocess=False, **default_kwargs)
